...
int main()
{
    ...
    addr.sin_addr.s_addr = inet_addr("172.31.28.107");//net_addr方法可以转化字符串，主要用来将一个十进制的数转化为二进制的数，用途多于ipv4的IP转化。
    cout << "2.准备通讯地址" << endl;
    ...
    cout << "3.bind ok 等待客户端的连接" << endl;

    //4.监听客户端listen()函数
    //参数二：进程上限，一般小于30
    cout << "4.监听客户端listen" << endl;
    listen(socket_fd,30);

    //5.等待客户端的连接accept()，返回用于交互的socket描述符
    cout << "5.等待客户端的连接accept()" << endl;
    struct sockaddr_in client;
    socklen_t len = sizeof(client);
    int fd = accept(socket_fd,(struct sockaddr*)&client,&len);
    cout << "5.返回用于交互的socket描述符" << endl;
    if (fd == -1)
    {
        cout << "accept错误\n" << endl;
        exit(-1);
    ...
    cout << "6.使用第5步返回socket描述符，进行读写通信,客户： 【" << ip << "】连接成功" << endl;
